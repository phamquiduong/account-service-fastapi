...
class WhiteListService:
    ...
    async def create(self, token_payload: TokenPayloadSchema) -> None:
        await self._dynamodb_service.add_item(token_payload.model_dump())

    async def get(self, token_payload: TokenPayloadSchema) -> dict | None:
        return await self._dynamodb_service.get_item(self._get_dynamodb_key(token_payload))
    ...
    async def delete(self, token_payload: TokenPayloadSchema) -> None:
        return await self._dynamodb_service.delete(self._get_dynamodb_key(token_payload))

    async def delete_by_user_id(self, user_id: str) -> None:
        await self._dynamodb_service.delete_by_partition_key(
            partition_key="sub", partition_value=user_id, sorted_key="jti"
        )
